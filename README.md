...
- 플롯 전용 대화 프로필: 플롯마다 전용 유저 페르소나(이름/이미지/짧은 소개/설명)를 개수 제한 없이 만들 수 있고, 새 채팅을 시작할 때 스와이프 카드로 먼저 고르거나 전체 프로필 목록으로 전환해서 고를 수 있음. 마이페이지/플롯 전용 대화 프로필 모두 비주얼 노벨 전용 스탠딩 이미지를 따로 설정 가능
- 비주얼 노벨 플레이어블 캐릭터: 캐릭터를 "플레이어블"로 지정해두면 비주얼 노벨에서 그 캐릭터를 내 페르소나로 선택해 진행할 수 있고, 선택하지 않은 다른 플레이어블 캐릭터는 이야기에 등장하지 않음
- 미니게임: 캐릭터 한 명을 골라 1:1로 체스·오목·우노·라이어스바(카드 블러핑 + 생명 게이지)를 즐길 수 있음. 체스/오목은 난이도 설정 가능, 모든 게임에서 CPU 알고리즘 대신 AI 프리셋이 수를 판단하게 하는 옵션과 상대가 매 수마다 성격에 맞는 대사를 하는 옵션을 켤 수 있음(둘 다 실패해도 자동으로 기존 방식으로 진행)
- `{{char}}`(캐릭터 소개글 안에서 자기 이름으로 치환) / `{{user}}`(대화 프로필 이름으로 실시간 치환) 매크로 지원
- 로어북(세계관/설정) 작성 및 플롯 연결 — 키워드가 언급되면 AI에게 자동 전달
  - 로어북 가져오기/내보내기: SillyTavern World Info JSON 또는 JanitorAI 스타일 로어북 스크립트를 가져올 수 있고(새 로어북으로, 또는 기존 로어북에 항목 추가), SillyTavern World Info JSON으로 내보내기 지원
- ZedTalk: 카카오톡처럼 캐릭터 한 명과 짧고 일상적인 톤으로 주고받는 1:1 메신저 채팅
  - 다중 캐릭터 플롯이면 시작 전 대화할 캐릭터를 먼저 선택
  - 플롯 대화와 동일한 햄버거 메뉴(새로 시작/다른 톡방 보기/삭제/대화 프로필/나가기)와 유저 말풍선 롱프레스 메뉴(여기서 새로 시작/복사/삭제)
  - 마지막 캐릭터 답장 수정·재시도, 말풍선 앞 캐릭터 프로필 이미지 표시
  - 카카오톡 스타일 날짜 구분선과 보낸 시각 표시, AI 컨텍스트에 실제 날짜/시간 실시간 반영
...
- 장기 기억(요약) 설정: 컨텍스트 길이를 넘는 오래된 대화를 자동 요약해서 시스템 프롬프트에 얹는 기존 기능을 마이페이지 → 요약 설정에서 켬/끔, 요약 프롬프트 커스텀, 요약 전용 프리셋 지정 가능
...
- SillyTavern 호환 카드 저장/불러오기 — 가져올 때 새 플롯으로 만들지, 기존 플롯에 캐릭터/로어북을 병합할지 선택 가능
- 플롯 전용 형식(.mzplot) 내보내기/불러오기 — 이미지를 포함해 플롯 하나의 모든 데이터(대화 기록 제외)를 손실 없이 내보내고, 새 플롯으로 만들거나 기존 플롯에 병합해서 불러옴
- 전체 데이터 백업/복원(.mzbackup), 환경설정에서 전체 초기화(공장 초기화)
- 환경설정에서 인트로/스냅샷 이미지 표시 방식(정사각형/가로 꽉 채우기) 선택
- 환경설정에서 다크/AMOLED(완전 검정) 테마 선택 (라이트 테마는 아직 실험적)
...
- **SillyTavern 카드**: 대표 캐릭터 1명 + 대표 이미지 1장 기준의 표준 호환 포맷. 다른 SillyTavern 계열 앱과 주고받을 때 사용합니다. 제작 탭의 "가져오기"로 불러올 때 새 플롯으로 만들지, 기존 플롯에 캐릭터로 병합할지 고를 수 있고, 카드에 로어북이 있으면 새 로어북을 만들지 그 플롯에 이미 연결된 로어북에 항목을 이어 붙일지도 고를 수 있습니다.
- **전용 형식(.mzplot)**: 플롯 편집 → `⋮` 메뉴 → "전용 형식으로 내보내기". 캐릭터 여러 명, 이미지, 인트로 여러 버전, 플롯 전용 프로필, 연결된 로어북까지 이미지를 포함해 그대로 내보냅니다(대화 기록은 제외). 제작 탭의 "가져오기"에서 다시 불러올 수 있으며, SillyTavern 카드와 마찬가지로 새 플롯으로 만들거나 기존 플롯/로어북에 병합해서 추가할 수 있습니다.
- **대화 프로필 전용 형식(.mzprofile)**: 마이페이지 → 대화 프로필 편집에서 프로필 하나를 열고 상단 공유 아이콘으로 이미지(초상화 + VN 스탠딩 이미지)를 포함해 통째로 내보냅니다. 프로필 목록 화면 상단 아이콘으로 다시 불러오면 항상 새 프로필로 추가됩니다.
- **로어북(SillyTavern World Info / JanitorAI)**: 로어북 편집 화면 상단 아이콘으로 SillyTavern World Info JSON(`{"entries": {...}}`) 또는 JanitorAI 스타일 로어북 스크립트(평평한 배열 JSON)를 가져오고, SillyTavern World Info JSON으로 내보낼 수 있습니다. 가져오기는 지금 열려 있는 로어북(새로 만드는 중이든 기존 로어북이든)에 항목으로 추가되고, 저장을 눌러야 실제로 반영됩니다.
